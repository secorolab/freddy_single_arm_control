...
namespace motion_specification_action
{
  ...
  public:
    ...
    explicit MotionSpecificationActionServer(const rclcpp::NodeOptions &options = rclcpp::NodeOptions())
        : Node("motion_specification_action_server", options), control_loop_active_(true)
    {
  ...
  private:
    ...
    std::atomic<bool> control_loop_active_; // Flag to control the control loop
    std::thread control_loop_thread_;       // Thread for the control loop
    volatile sig_atomic_t flag = 0;         // used if handling keyboard interrupt
    ...
    void check_if_motion_specification_is_read(YAML::Node &motion_specification_params,
                                               const std::string &arm_name,
                                               int &motion_specification_read,
    ...
    // manage memory between both threads
  ...
  }; // class MotionSpecificationActionServer
